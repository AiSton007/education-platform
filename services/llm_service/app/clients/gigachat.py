...
2. POST ``/chat/completions`` to ``LLM_API_URL`` with ``Authorization: Bearer <token>`` and a
   prompt that instructs the model to return strict JSON.
...
class GigaChatAnalyzer:
    ...
    async def _ensure_token(self) -> str:
        ...
        # Some responses use ``expires_at`` (ms), others ``expires_in`` (s).
    ...
    async def analyze(self, prompt: str) -> tuple[AnalysisResult, dict]:
            ...
            "messages": [
                {
                    ...
                    "content": (
                        ...
                        "Отвечай ТОЛЬКО валидным JSON в указанной схеме."
                    ),
                },
        ...
        result = _to_analysis_result(parsed)
        return result, raw
...
def _to_analysis_result(parsed: dict[str, Any]) -> AnalysisResult:
